...
def transform(resultado_json: dict) -> pd.DataFrame:
    ...
    for s in solicitacoes:
        ...
        resp_tec_data = {f"responsaveltecnico_{k}": v for k, v in s.pop("responsaveltecnico", {}).items()} # noqa

        solicitacao_row = {**s, **paciente_data, **resp_tec_data, **lote_info}
        solicitacao_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"{solicitacao_row.get('codigoLis', '')}|{solicitacao_row.get('dataPedido', '')}|{solicitacao_row.get('paciente_nome', '')}")) # noqa
        ...
        for e in exames_data:
            ...
            exame_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"{solicitacao_id}|{exame_row.get('codigoApoio', '')}|{exame_row.get('dataAssinatura', '')}")) # noqa
            ...
            for r in resultados_data:
                ...
                resultado_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"{exame_id}|{resultado_row.get('codigoApoio', '')}|{resultado_row.get('descricaoApoio', '')}")) # noqa
    ...
    solicitacoes_df = solicitacoes_df.drop(columns=["paciente", "responsaveltecnico"], errors="ignore") # noqa
