...
from pipelines.datalake.extract_load.medlab_api.schedules import (
    medlab_api_schedule,
)
from pipelines.datalake.extract_load.medlab_api.constants import (
    medlab_api_constants,
)
...
with Flow(
    ...
    API_USUARIO = get_secret_key(
        secret_path=INFISICAL_PATH, 
        secret_name="API_USUARIO", 
        environment=ENVIRONMENT
    )

    API_SENHA = get_secret_key(
        secret_path=INFISICAL_PATH, 
        secret_name="API_SENHA", 
        environment=ENVIRONMENT
    )

    API_CODACESSO = get_secret_key(
        secret_path=INFISICAL_PATH, 
        secret_name="API_CODACESSO", 
        environment=ENVIRONMENT
    )
