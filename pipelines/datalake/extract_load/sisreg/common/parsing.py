...
def assertar_landmark(
    html: str,
    seletor_css: Optional[str] = None,
    id_elemento: Optional[str] = None,
    conjunto: str = "",
    item: str = "",
) -> None:
    """Valida que um landmark DOM esta presente antes de tentar parsear.

    Levanta ErroEstrutura se o elemento nao for encontrado. Usar antes de
    qualquer acesso a soup.find() que assuma a existencia de um elemento -
    assim elementos renomeados no SISREG viram erros nomeados, nao IndexError.

    Args:
        html: Texto HTML a verificar.
        seletor_css: Seletor CSS (ex.: "textarea#preparo").
        id_elemento: Atributo id do elemento (alternativo ao seletor).
        conjunto: Contexto de erro.
        item: Contexto de erro.
    """
    soup = BeautifulSoup(html, "lxml")
    ...
    detalhe = ""
    if seletor_css:
        encontrado = soup.select_one(seletor_css) is not None
        detalhe = seletor_css
    elif id_elemento:
        encontrado = soup.find(id=id_elemento) is not None
        detalhe = f"id={id_elemento}"

    if not encontrado:
        raise ErroEstrutura(
            f"Landmark ausente: {detalhe}",
            conjunto=conjunto,
            etapa="parsing",
            item=item,
            detalhe=f"Elemento '{detalhe}' nao encontrado - provavel mudanca no SISREG",
        )
