...
def get_elasticsearch_client(user: str, password: str) -> Elasticsearch:
...
@task(max_retries=3, retry_delay=timedelta(minutes=5))
def initiate_session(user: str, password: str, request_delay: float) -> requests.Session:
...
@task(max_retries=3, retry_delay=timedelta(minutes=5))
def get_procedimentos(es: Elasticsearch, max_es_pages: int = None) -> pd.DataFrame:
    ...
    # df.to_excel('debug_ppi.xlsx', index=False)
    return df
...
def extract_vagas_info(
    ...
    for idx, df_row in df_procedimentos.iterrows():
        ...
        if _verify_text("INEXISTENTES!", soup_page_text):
            resultados.append({
                ...
                "qtd_pend": None,
                "qtd_vagas": None,
                "dt_extracao": datetime.now().strftime("%Y-%m-%d"),
        ...
        if _verify_text("NENHUMA VAGA ENCONTRADA", soup_page_text) or _verify_text("encontra-se autorizada.", soup_page_text):
            resultados.append({
                "cod_interno_proced": codigo_interno,
                "nm_proced": nome_proced,
                "cnes_unidade": None,
        ...
        for k, unidade in enumerate(unidades):
            ...
            for row in rows:
                ...
                # garantir que tem todos os campos esperados
                if len(row_data) < 6:
                    continue
    ...
    # Handle empty vagas_detalhadas list by maintaining proper schema
    if vagas_detalhadas:
        df_vagas_detalhes = pd.DataFrame(vagas_detalhadas)
    else:
        # Create empty DataFrame with proper schema to avoid crashes downstream
