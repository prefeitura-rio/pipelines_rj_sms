...
@task(max_retries=3, retry_delay=timedelta(minutes=5))
def processar_unidades(session: Session, unidades: List[Tag], unidades_limit: int | None = None) -> pd.DataFrame:

    unidades_iter = unidades[:unidades_limit] if unidades_limit else unidades  
