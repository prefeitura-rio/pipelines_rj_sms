...
from pipelines.datalake.extract_load.sisreg_solicitacoes.constants import (
    BASE_URL,
    GERENCIADOR_URL,
    HEADERS_DISFARCE,
    RUN_CONFIGS,
...
# all_finished garante que o pipeline guarde os successes mesmo se houver falhas irrecuperáveis
@task(trigger=all_finished)
def consolidar_e_salvar(lista_de_dfs: list, dataset_id: str, table_id: str) -> None:
    if lista_de_dfs is None or not isinstance(lista_de_dfs, list):
        log("❌ A consolidação foi abortada porque os dados upstream falharam ou retornaram Nulo.")
        return
