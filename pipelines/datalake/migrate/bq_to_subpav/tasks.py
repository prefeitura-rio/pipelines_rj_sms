...
@task
def resolve_project_id(environment: str) -> str:
    """
        Retorna o project id do enviroment evitando o parametro manual
    """
    return get_project_id(environment)
...
def query_bq_table(
    ...
    try:
        ...
        log(f"[{environment.upper()}] {full_ref} → {len(df)} registros", 
            level="info")
        return {"df": df, "metrics": None}
    except Exception as e:
        msg = f"[{environment.upper()}] Erro ao consultar BigQuery ({full_ref}): {type(e).__name__}: {e}"
        log(msg, level="error")
...
def insert_df_into_mysql(
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            if custom_insert_query:
                ...
                df = ensure_dataframe_columns(df, required_columns, fill_value=None, notes=metrics["notes"])

                campos_none = [col for col in required_columns if df[col].isnull().all()]
                if campos_none:
                    warn_msg = f"Atenção: as colunas {', '.join(campos_none)} foram preenchidas somente com None (cheque o schema de destino)."
    ...
    metrics["execution_time"] = (datetime.now() - start).total_seconds()
    log(
        f"[{environment.upper()}] inserção finalizada: inseridos={metrics['inserted']}, falhas={metrics['failed']}, tempo={metrics['execution_time']:.2f}s",
        level="info",
    )
    return metrics
