...
def build_db_url(environment: str):
    ...
    password = get_secret_key.run(secret_path="/", secret_name="DB_PASSWORD", environment=environment)
...
@task(max_retries=3, retry_delay=timedelta(seconds=90))
def get_mergeable_data_from_db(
    limit: int,
    offset: int,
    db_url: str
) -> pd.DataFrame:
...
def merge_patientrecords(mergeable_data: pd.DataFrame):
    ...
    mergeable_data.loc[
        mergeable_data['birth_date'].notna(),
        'birth_date'
    ] = mergeable_data.loc[
        mergeable_data['birth_date'].notna(),
        'birth_date'
    ...
    mergeable_data.loc[
        mergeable_data['deceased_date'].notna(),
        'deceased_date'
    ] = mergeable_data.loc[
        mergeable_data['deceased_date'].notna(), 
        'deceased_date'
    ...
    grouped = mergeable_data.groupby('patient_code').apply(
