...
def clean_null_values(df: pd.DataFrame, endpoint: str):
    ...
    def normalize_list(row,endpoint):
        for dic in row:
            req_fields = [i for i in dic.keys() if i in datalake_constants.ENDPOINT_REQUIRED_FIELDS.value[endpoint]]
            dic['is_valid']=True
    ...
    if endpoint == "mrg/professionals":
        ...
        payload = df[
        ["id_profissional_sus", "cpf", "cns", "nome", "cbo", "conselho"]].to_dict(orient="records")
