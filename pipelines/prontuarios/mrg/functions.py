...
def first_merge(group: dict,ranking_df: pd.DataFrame) -> Tuple[dict, dict]:
    fields_to_pass = ['event_moment', 'ingestion_moment', 'active', 'raw_source_id', 'is_valid', 'system', 'cnes', 'description']
    merged_payload = {}
    not_merged_payload = {}
    ...
    if len(not_merged_payload) == 0:
        ...
        for key,values in not_merged_payload.items():
            ...
            if merging_type == 'append':
                ...
                merged_payload[key] = [c for l,c in values.loc[values['ranking']==1,key].items()][0]
            else:
...
def sanity_check(merged_payload: dict) -> dict:
    ...
    if merged_payload['cns_list'] is not None:
        ...
        if len(cns_values) != len(set(cns_values)): 
            ...
            for cns_dic in merged_payload['cns_list']:
                if (cns_dic['value'] in duplicated_cns) & (cns_dic['is_main'] ==True):
                    unique_cns_dics.remove(cns_dic)
            merged_payload['cns_list'] = unique_cns_dics
                    
        main_cns = [cns for cns in merged_payload['cns_list'] if (cns['is_main'] == True)]
        if len(main_cns) > 1:
    ...
    if merged_payload['address_list'] is not None: # Essa validação precisa ser feita em std e não foi
