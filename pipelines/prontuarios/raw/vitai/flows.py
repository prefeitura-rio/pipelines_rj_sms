...
with Flow(
    name="Prontuários (Vitai) - Extração de Dados: Paciente",
) as vitai_patients:
    #####################################
    # Parameters
    #####################################
...
vitai_conditions.executor = LocalDaskExecutor(num_workers=5)
...
vitai_patients.schedule = vitai_daily_update_schedule
vitai_patients.storage = GCS(constants.GCS_FLOWS_BUCKET.value)
vitai_patients.executor = LocalDaskExecutor(num_workers=5)
