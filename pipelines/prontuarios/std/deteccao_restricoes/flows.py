...
from pipelines.prontuarios.std.deteccao_restricoes.constants import hci_pacientes_restritos_constants
from pipelines.prontuarios.std.deteccao_restricoes.schedules import hci_pacientes_restritos_schedules
...
with Flow(name="HCI - Transformação - Detecção de pacientes restritos") as hci_pacientes_restritos:
    #####################################
    # Parameters
    #####################################
    ...
    motivo_atendimento_reduced,usuario_rastreado = reduce_raw_column(df=query_results)
    list_result = get_result_gemini.map(atendimento=motivo_atendimento_reduced, 
                                        flag_rastreio=usuario_rastreado, 
                                        env=ENVIRONMENT)
...
hci_pacientes_restritos.schedule = hci_pacientes_restritos_schedules
