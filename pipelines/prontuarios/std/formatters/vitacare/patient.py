...
def standardize_parents_names(data: dict) -> dict:
    ...
    Returns:
        data (dict) : Individual data record standardized
    """
    data["mother_name"] = clean_name_fields(
        data["nomeMae"]) if clean_name_fields(data["nomeMae"]) != '' else None
    data["father_name"] = clean_name_fields(
        data["nomePai"]) if clean_name_fields(data["nomePai"]) != '' else None
    return data
