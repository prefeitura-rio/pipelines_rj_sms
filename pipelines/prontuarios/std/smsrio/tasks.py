...
def define_constants() -> Tuple[list, dict, dict, dict, dict]:
    ...
    '''
    lista_campos_api = ["active", "birth_city_cod", "birth_state_cod",
                        "birth_country_cod", "birth_date", "patient_cpf",
                        "deceased", "deceased_date", "father_name", "gender",
                        "mother_name", "name", "nationality", "protected_person",
                        "race", "cns_list", "address_list", "telecom_list", "raw_source_id"]

    logradouros = pd.read_csv('pipelines/prontuarios/std/logradouros.csv',
                              dtype={'Número_DNE': str, 'Nome': str})
    logradouros_dict = dict(zip(logradouros['Número_DNE'], logradouros['Nome']))

    city = pd.read_csv('pipelines/prontuarios/std/municipios.csv',
                       dtype={'COD UF': str, 'COD': str, 'NOME': str})
    city_dict = dict(zip(city['COD'].str.slice(start=0, stop=-1), city['COD']))

    state = pd.read_csv('pipelines/prontuarios/std/estados.csv',
                        dtype={'COD': str, 'NOME': str, 'SIGLA': str})
    state_dict = dict(zip(state['SIGLA'], state['COD']))

    countries = pd.read_csv('pipelines/prontuarios/std/paises.csv',
...
def clean_records_fields(data: dict, lista_campos_api: list) -> dict:
    ...
    '''
    data = dict(filter(lambda item: (item[1] is not None) &
                       (item[0] in lista_campos_api), data.items()))
    return data
...
def standardize_cns_list(data: dict) -> dict:
    ...
    if (len(lista) == 1) & (lista[0] is None):
        ...
        data['cns_list'] = [dic_cns_value(lista[0], True)] + \
...
@task
def standardize_address_data(data: dict,
