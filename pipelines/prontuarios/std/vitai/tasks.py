...
@task
def load_from_api(params: dict, endpoint_name: str, api_token: str, environment: str) -> None:
    """
    ...
    request_response = requests.get(
        url=url, headers={"Authorization": f"Bearer {api_token}"}, timeout=90
    )
...
def standartize_data(
    ...
    for patient in data:
        ...
        patient_std = {
            ...
            "birth_country_cod": "1",
            "birth_date": str(
                pd.to_datetime(patient["data"]["dataNascimento"], format="%Y-%m-%d %H:%M:%S").date()
            )
            if patient["data"]["dataNascimento"] != None
            else "0",
            ...
            "deceased_date": str(
                pd.to_datetime(patient["data"]["dataObito"], format="%Y-%m-%d %H:%M:%S").date()
            )
            if patient["data"]["dataObito"] != None
            else None,
            "father_name": patient["data"]["nomePai"].title()
            if patient["data"]["nomePai"] is not None
            else None,
            "gender": gender_validation(patient["data"]["sexo"]),
            "mother_name": patient["data"]["nomeMae"].title()
            if patient["data"]["nomeMae"] is not None
            else None,
            ...
            "race": patient["data"]["racaCor"].lower()
            if patient["data"]["racaCor"] is not None
            else None,
            "cns_list": [
            ...
            "address_list": [
                {  # adaptar para varios end
                    "use": None,
                    "type": None,
                    "line": patient["data"]["tipoLogradouro"].title()
                    + " "
                    + patient["data"]["nomeLogradouro"].title()
                    if (
                        (patient["data"]["tipoLogradouro"] != None)
                        & (patient["data"]["nomeLogradouro"] != None)
                    )
                    else "0",
            ...
            "telecom_list": [
                {
                    "system": None,
                    "use": None,
                    "value": patient["data"]["telefone"]
                    if patient["data"]["telefone"] is not None
                    else "0",
                    "rank": 0,
                    "start": None,
                    "end": None,
...
def load_to_api(request_body: dict, endpoint_name: str, api_token: str, environment: str) -> None:
    ...
    request_response = requests.post(
        url=url, headers={"Authorization": f"Bearer {api_token}"}, timeout=90, json=request_body
    )
