...
def create_allergie_list(dataframe_allergies_vitai: pd.DataFrame):
    ...
    def clean_allergies_field(allergies_field):
        allergies_field_clean = re.sub(
            "INTOLERANCIA A{0,1}|((REFERE ){0,1}ALERGIA A{0,1})|(PACIENTE ){0,1}AL[É|E]RGIC[A|O] AO{0,1}",
            "",
            allergies_field,
        )
        allergies_field_clean = re.sub(
            ".*N[Ã|A]O [(SABE)|(RECORDA)|(LEMBRA))].*", "", allergies_field_clean
        )
    ...
    dataframe_allergies_vitai["text_input_clean"] = dataframe_allergies_vitai["agg_alergia"].apply(
        lambda x: clean_allergies_field(x)
    )
