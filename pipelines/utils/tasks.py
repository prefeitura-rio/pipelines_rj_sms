...
def upload_df_to_datalake(
    ...
    log(f"DEBUG: upload_df_to_datalake iniciado. Tamanho do DataFrame de entrada: {df.shape[0]} linhas.", level="info")
    log(f"DEBUG: df.empty no início da tarefa: {df.empty}", level="info")
    if df.empty:
        log("DEBUG: DataFrame vazio detectado na verificação inicial. Levandando exceção.", level="error")
    ...
    if not os.path.isdir(root_folder):
        log(f"DEBUG ERROR: O diretório temporário '{root_folder}' NÃO foi criado com sucesso!", level="error")
    ...
    try:
        ...
        log(f"DEBUG ERROR: Falha no teste de escrita no diretório '{root_folder}': {e}", level="error")
    ...
    if partition_column:
        ...
        log(f"DEBUG: Chamando create_date_partitions. DataFrame possui {df.shape[0]} linhas.", level="info")
        ...
        if not os.path.isdir(partition_folder):
            log(f"DEBUG ERROR: O diretório de partição '{partition_folder}' NÃO foi criado ou não existe após create_date_partitions!", level="error")
        ...
        if source_format == "csv":
            ...
            log(f"DEBUG: Tentando salvar Parquet. DataFrame possui {df.shape[0]} linhas.", level="info")
            safe_export_df_to_parquet.run(df=df, output_path=file_path)

        # ✅ Verificação se o arquivo foi realmente criado
        if not os.path.exists(file_path):
            log(f"DEBUG ERROR: Arquivo '{file_path}' NÃO foi criado após a tentativa de escrita!", level="error")
            raise FileNotFoundError(f"Arquivo não foi criado em {file_path}")
        else:
            log(f"DEBUG: Arquivo '{file_path}' criado com sucesso e encontrado no sistema de arquivos.", level="info")
            ...
            if file_size == 0 and df.shape[0] > 0: # Se o DF não é vazio mas o arquivo é
                log(f"DEBUG ERROR: Arquivo '{file_path}' está vazio, mas o DataFrame original possuía {df.shape[0]} linhas!", level="error")

        partition_folder = root_folder

    log(f"Uploading data to partition folder: {partition_folder}")
    files_in_folder = glob.glob(os.path.join(partition_folder, f"**/*.{source_format}"), recursive=True)
    log(f"DEBUG: Quantidade de arquivos {source_format} encontrados em '{partition_folder}' ANTES de chamar upload_to_datalake: {len(files_in_folder)}.", level="info")
    if len(files_in_folder) == 0 and df.shape[0] > 0:
         log("DEBUG ERROR: Nenhum arquivo encontrado para upload, mas o DataFrame não era vazio!", level="error")
